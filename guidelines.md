...
|---|-------|------|--------|--------|----------|
| P1 | REQ | Paper format: DIN A5 (148 × 210 mm) | KSP p.6 | ✅ | `page-dimensions-by-format` in `src/page-conf.typ`, `setup-page()` in `src/page-setup.typ` |
| P2 | REQ | All pages must be in portrait orientation | KSP p.3 | ✅ | `page-dimensions-by-format` in `src/page-conf.typ` (148 mm wide × 210 mm tall — A5 is natively portrait) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| F1 | REQ | Computer Modern (cm, cmr, cms…) and LM fonts must **not** be used | KSP p.1 | ✅ | `fonts` in `src/typography.typ` (Libertinus family only) |
| F2 | REQ | All fonts must be embedded in the final PDF | KSP p.1 | ✅ | Typst embeds all fonts by default at compile time |
| F3 | REQ | Base font size: 10 pt for DIN A5 (no scaling) | KSP p.6 | ✅ | `font-sizes-by-format` in `src/typography.typ`, `setup-page()` in `src/page-setup.typ` |
| F4 | REC | Approved serif fonts (body): Libertinus Serif, Nimbus Roman, URWPalladio, Utopia Roman | KSP p.6 | ✅ | `fonts.serif` in `src/typography.typ` |
| F5 | REC | Approved sans-serif font (headings): Nimbus Sans | KSP p.6 | ⚠️ | `fonts.sans` / `fonts.mono` in `src/typography.typ` — defaults to Libertinus Sans (visually comparable); `serif-headings: true` switches to Libertinus Serif for institutions requiring serif headings |
...
|---------|------|--------|----------|
| H1 / chapter | 18 pt | ✅ | `font-sizes-by-format` in `src/typography.typ`, `setup-headings()` in `src/headings.typ` |
| H2 / section | 14 pt | ✅ | `font-sizes-by-format.*.base` in `src/typography.typ`, `setup-headings()` in `src/headings.typ` |
| H3 / subsection | 12 pt | ✅ | `font-sizes-by-format.*.chapter` in `src/typography.typ`, `setup-headings()` in `src/headings.typ` |
| H4 / subsubsection | 10 pt | ✅ | `font-sizes-by-format.*.section` in `src/typography.typ`, `setup-headings()` in `src/headings.typ` |
| Body text | 10 pt | ✅ | `font-sizes-by-format` in `src/typography.typ`, the `set text` in `setup-page()`, `src/page-setup.typ` |
| Headers, footnotes, captions | 8 pt | ✅ | `font-sizes-by-format.*.footnote` in `src/typography.typ`, `_header()` in `src/page-setup.typ`, `setup-figures()` in `src/figures.typ`, and the `show footnote.entry` in `setup-page()` |
...
|-----------|---------------------|-------|--------|----------|
| up to 199 pages | 20 mm | 15–18 mm | ✅ | `margins-by-format.a5.short` in `src/page-conf.typ` (`inside: 20mm, outside: 15mm`) |
| 200–399 pages | 23 mm | 15–18 mm | ✅ | `margins-by-format.a5.medium` in `src/page-conf.typ` (`inside: 23mm, outside: 15mm`) |
| 400 pages and above | 25 mm | 15 mm | ✅ | `margins-by-format.a5.long` in `src/page-conf.typ` (`inside: 25mm, outside: 15mm`) |

All measurements are from the paper edge, including above headers and below pagination.

| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| M1 | REQ | Inner margin must always be wider than outer margin | KSP p.4 | ✅ | `margins-by-format` in `src/page-conf.typ` (inside 20/23/25 mm > outside 15 mm for all presets) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| H1 | REQ | Header on all pages with content | KSP p.2 | ✅ | `_header()` in `src/page-setup.typ` (`_header`) |
| H2 | REQ | Left (even) page: chapter title | KSP p.2 | ✅ | `_header()` in `src/page-setup.typ` |
| H3 | REQ | Right (odd) page: section heading | KSP p.2 | ✅ | `_header()` in `src/page-setup.typ` |
| H4 | REQ | No header on the first page of a chapter | KSP p.2 | ✅ | `_header()` in `src/page-setup.typ` (suppressed when a level-1 heading is on the current page) |
| H5 | REQ | Chapters without sub-sections: chapter title on both pages | KSP p.2 | ✅ | `_header()` in `src/page-setup.typ` (falls back to chapter label when no sections found) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| G1 | REQ | Always ragged (left-aligned), never justified | KSP p.2 | ✅ | `setup-headings()` in `src/headings.typ` (`set par(justify: false)`) |
| G2 | REQ | Multiple consecutive headings indented to the same vertical height | KSP p.2 | ✅ | Typst's built-in heading numbering indent handles this; `setup-headings()` in `src/headings.typ` |
| G3 | REQ | 4th-level indent must not exceed 5 mm | KSP p.2 | ✅ | `setup-headings()` in `src/headings.typ` (headings have no extra indent; uses only the pixel-measured numbering width) |
| G4 | REC | Set in bold | KSP p.2 | ✅ | `setup-headings()` in `src/headings.typ` (`weight: "bold"` for all levels) |
| G5 | REC | No hyphenation in headings | KSP p.2 | ✅ | `setup-headings()` in `src/headings.typ` (`hyphenate: false` for all levels) |
| G6 | REC | Second line indented to the height of the first | KSP p.2 | ✅ | Handled naturally by Typst's `block` text wrapping |
| G7 | REC | Font sizes must differ by at least 2 pt between levels | KSP p.2 | ✅ | `font-sizes-by-format` in `src/typography.typ` (18 → 14 → 12 → 10 pt; minimum gap is 2 pt) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| B1 | REQ | Body text in justification (Blocksatz) | KSP p.1 | ✅ | the `set par` in `setup-page()`, `src/page-setup.typ` (`justify: true`) |
| B2 | REQ | Body text 100 % black | KSP p.1 | ✅ | Typst default; no fill override applied to body text |
| B3 | REC | Line spacing 1.15–1.2 × (`\setstretch{1.15}` in LaTeX) | KSP p.4 | ✅ | `leading` in `src/typography.typ` (`leading = 0.75em` ≈ 1.15×), the `set par` in `setup-page()`, `src/page-setup.typ` |
| B4 | REC | Delete all first-line indents; use paragraph spacing instead | KSP p.3 | ✅ | the `set par` in `setup-page()`, `src/page-setup.typ` (`first-line-indent: 0pt`), `par-spacing` in `src/page-conf.typ` (`par-spacing = 1.0em`) |
| B5 | NTH | Do not indent first line after blank lines or at top of page | KSP p.3 | ✅ | the `set par` in `setup-page()`, `src/page-setup.typ` (`first-line-indent: 0pt` applied globally) |
| B6 | REQ | Automatic hyphenation enabled | KSP p.3 | ✅ | Typst enables hyphenation by default for justified text; the `set text` in `setup-page()`, `src/page-setup.typ` |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| FN1 | REQ | Footnotes 2 pt smaller than body text (8 pt for 10 pt base) | KSP p.2 | ✅ | `font-sizes-by-format.*.footnote` in `src/typography.typ` (`footnote: 8pt`), the `show footnote.entry` in `setup-page()`, `src/page-setup.typ` |
| FN2 | REQ | Numbers superscript in running text and front-aligned in footnote list | KSP p.2 | ✅ | Superscript in text: Typst default; front-aligned in list: the `show footnote.entry` in `setup-page()`, `src/page-setup.typ` (custom grid layout) |
...
| FN4 | REC | Align footnote text from second line with height of first line | KSP p.2 | ✅ | the `show footnote.entry` in `setup-page()`, `src/page-setup.typ` (grid layout: superscript number in `auto`-width column + 0.3em gutter, body text fills remainder) |
| FN5 | REC | Do not allow footnotes to span across pages | KSP p.2 | ✅ | Typst does not split footnotes across pages by default |
| FN6 | NTH | Restart footnote numbering for each chapter | KSP p.2 | ✅ | `setup-headings()` in `src/headings.typ` (`counter(footnote).update(0)` in level-1 branch of heading show rule) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| FT1 | REQ | Long captions (≥ 2 lines): left-justified | KSP p.3 | ✅ | `setup-figures()` in `src/figures.typ` (height threshold: `font-sizes.small * 1.5`; printed spec says ≥ 3 lines, but ≥ 2 is correect) |
...
| FT4 | REQ | Caption numbering includes chapter number (e.g. Figure 2.3) | KSP p.3 | ✅ | `setup-figures()` in `src/figures.typ` (`numbering("1.1", ch, it)`) |
| FT5 | REC | 1-line captions: centred (exception: if line spans nearly full width, left-align) | KSP p.3 | ✅ | `setup-figures()` in `src/figures.typ` (center for 1-line captions, left for ≥ 2 lines) |
...
| FM2 | REC | Formula numbers right-justified at the type-area margin | KSP p.2 | ✅ | the `set math.equation` in `setup-page()`, `src/page-setup.typ` (Typst places numbers right-aligned) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| E1 | REQ | Items ≥ 3 lines: justified; second line aligned to height of first | KSP p.1 | ✅ | Inherits `justify: true` from global `set par`; `_draft-indicator()` in `src/page-setup.typ` |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| L1 | REC | All lines (rules, borders) at least 0.3 pt (0.1 mm) in final format | KSP p.4 | ✅ | the rule at the end of `_header()`, `src/page-setup.typ` (header rule: `stroke: 0.3pt`) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| BM1 | REQ | Comments and links set to "not printing" | KSP p.1 | ⚠️ | Typst renders links as inline colored text (not PDF annotation objects), so they always print. Use `colored-links: false` to make them visually indistinguishable from body text; the `set par` in `setup-page()`, `src/page-setup.typ` |
