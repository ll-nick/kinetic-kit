...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| N1 | REQ | Page numbers on every page except blank pages | KSP p.4 | ✅ | `src/page-setup.typ:136-149` (footer); blank pages: `src/page-setup.typ:245-248`; title page: `src/title-page.typ:55` |
...
| H10 | REQ | Blank pages have no header | KSP p.2 | ✅ | `src/page-setup.typ:245-248` (`set page(header: none, footer: none)` scoped to the pagebreak that creates blank pages) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| G1 | REQ | Always ragged (left-aligned), never justified | KSP p.2 | ✅ | `src/page-setup.typ:252` (`set par(justify: false)` inside the heading block) |
| G2 | REQ | Multiple consecutive headings indented to the same vertical height | KSP p.2 | ✅ | Typst's built-in heading numbering indent handles this; `src/page-setup.typ:174-262` |
| G3 | REQ | 4th-level indent must not exceed 5 mm | KSP p.2 | ✅ | `src/page-setup.typ:178-262` (headings have no extra indent; uses only the pixel-measured numbering width) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| C1 | REQ | New chapters always begin on a right-hand (odd) page; insert blank page if necessary | KSP p.1 | ✅ | `src/page-setup.typ:247` (`pagebreak(weak: true, to: "odd")`) |
| C2 | REQ | Blank pages contain no page numbers and no headers | KSP p.1 | ✅ | `src/page-setup.typ:245-248` (`set page(header: none, footer: none)` applied in the same scope as the pagebreak, covering inserted blank pages) |
| C3 | REQ | Page breaks used for new chapters only, not for subsections | KSP p.1 | ✅ | Template only inserts pagebreaks in the level-1 branch of the unified heading show rule; `src/page-setup.typ:226-262` |
...
| FN6 | NTH | Restart footnote numbering for each chapter | KSP p.2 | ✅ | `src/page-setup.typ:240-244` (`counter(footnote).update(0)` in level-1 branch of heading show rule) |
...
| # | Level | Rule | Source | Status | Code ref |
|---|-------|------|--------|--------|----------|
| FT1 | REQ | Long captions (≥ 2 lines): left-justified | KSP p.3 | ✅ | `src/page-setup.typ:280-295` (height threshold: `font-sizes.small * 1.5`; printed spec says ≥ 3 lines, but ≥ 2 is correect) |
...
| FT4 | REQ | Caption numbering includes chapter number (e.g. Figure 2.3) | KSP p.3 | ✅ | `src/page-setup.typ:310-313` (`numbering("1.1", ch, it)`) |
| FT5 | REC | 1-line captions: centred (exception: if line spans nearly full width, left-align) | KSP p.3 | ✅ | `src/page-setup.typ:289-294` (center for 1-line captions, left for ≥ 2 lines) |
...
| FM2 | REC | Formula numbers right-justified at the type-area margin | KSP p.2 | ✅ | `src/page-setup.typ:319-322` (equation numbering set; Typst places numbers right-aligned) |
...
| T10 | REC | No hyphenation in TOC entries | KSP p.5 | ✅ | `src/outlines.typ:55` (`set text(hyphenate: false)` in `table-of-contents`) |
